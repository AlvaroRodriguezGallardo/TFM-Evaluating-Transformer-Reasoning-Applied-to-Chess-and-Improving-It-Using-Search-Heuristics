...
class AlgorithmProposed(ThinkMoreTemplate):
        ...
        # root_moves: MOVE):
         ...
         #   top_move = None
            ...
            for move in movs_preevaled:
                ...
                logging.info(f'eval: {eval} for move: {move}')
                board.pop()
    ...
    def get_best_moves(self, board: chess.Board, maximize_player):
            ...
            logging.info("debug Número de movs preevaluados: %s", len(movs_preevaled))
            if self.iter_method == "N_best":
                assert self.N_best is not None
                movs_for_iter = [move for move, _ in movs_preevaled[:self.N_best]]
                
            elif self.iter_method == "percentage":
                assert self.percentage is not None
                logging.info(f'debug Percentage: {self.percentage}')
                logging.info(f'debug N_moves to get: {int(len(movs_preevaled) * self.percentage)}')
                n_moves_to_get = int(len(movs_preevaled) * self.percentage)
                if n_moves_to_get == 0:         # Si es demasiado pequeño, nos quedamos con 1
                    n_moves_to_get = 1
                movs_for_iter = [move for move, _ in movs_preevaled[:n_moves_to_get]]

            elif self.iter_method == "percentage_epsilon":
                assert self.percentage_epsilon is not None
                epsilon = self.percentage_epsilon*max(win_probs_iter)
                ...
                logging.info(f'debug Epsilon: {self.percentage_epsilon}')
                logging.info(f'debug Epsilon value: {epsilon}')
                logging.info(f'debug N_moves to get: {len(movs_for_iter)}')
                if len(movs_for_iter) == 0:         # Si es demasiado pequeño, nos quedamos con 1
                    movs_for_iter = [move for move, _ in movs_preevaled[:1]]
