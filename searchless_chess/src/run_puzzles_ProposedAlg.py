...
def get_engine(agent: str,N_preevals: int, depth: int, reeval_level: int, iter_search_method: str = None, N_best = None, percentage = None, percentage_epsilon = None) -> engine_lib.Engine:
    ...
    if _AGENT.value == 'AlgorithmProposedIterative':   
      return constants.get_algorithm_proposed(depth=depth,reeval_level=reeval_level,N_preevals=N_preevals, method=iter_search_method, N_best=N_best, percentage=percentage, percentage_epsilon=percentage_epsilon)
