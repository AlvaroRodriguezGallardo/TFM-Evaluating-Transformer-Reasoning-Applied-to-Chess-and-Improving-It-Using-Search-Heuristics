...
def extract_and_save_data(input, mode, output_name):
    ...
    if mode == 'Full':
        ...
        patron_preeval = re.compile(
            r'Number of moves preevaluated:\s*(\d+)',
            re.IGNORECASE
        )

        preeval = None  # Valor por defecto

        with open(input, 'r', encoding='utf-8') as f:
            for linea in f:
                match_preeval = patron_preeval.search(linea)
                if match_preeval:
                    preeval = int(match_preeval.group(1))
                    continue
                ...
                if 'Time for algorithm' in linea:
                    ...
                    if match:
                        ...
                        data.append([profundidad, reeval, tiempo, preeval])
                        preeval = None  # Reiniciar después de usar

        headers = ['Depth', 'Reeval_level', 'Time(s)', 'Preeval']
        ...
        with open(input, 'r', encoding='utf-8') as f:
            for linea in f:
                ...
                if match_tiempo:
                    ...
                    preeval = None  # Reiniciar tras usar

        headers = ['Depth', 'Reeval_level', 'N_best', 'percentage', 'percentage_epsilon', 'Time(s)', 'Preeval']
...
if __name__ == '__main__':
    if len(sys.argv) != 4:
        print("Use: python extract_info_for_slurms.py <fichero.txt> <Full|IterativeSearch> <output_name>")
    else:
        input = sys.argv[1]
        mode = sys.argv[2]
